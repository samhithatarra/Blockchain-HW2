...
class Blockchain(persistent.Persistent):
    ...
    def get_chain_ending_with(self, block_hash):
        ...
        if block_hash in self.blocks.keys():
            ...
            return []
        chain = [block.calculate_hash()]

        while not block.is_genesis:
            if block.calculate_hash() not in chain:
                chain.append(block.calculate_hash())
            block = self.blocks[block.parent_hash]
        if block.calculate_hash() not in chain:
            chain.append(block.calculate_hash())
        
        sorted_chain = sorted(chain, key=lambda x: self.blocks[x].get_weight())
