...
while try_block <= MAX_BLOCK_TO_GENERATE:
    try_block += 1
    chain = chaindb.chain
    txs = []
    ...
    if curr_height < 10:
        num_txs = min(MAX_TXS_PER_BLOCK, num_txs + 100) # seed early blocks with lots of txs to prevent duplicate hashes
